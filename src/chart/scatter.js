...
export default function (leftVal, rightVal, prefOrCity, palentDiv) {
  ...
  const isEStat = leftVal.estat;
  const rangeDiv = d3.select('#year-range-scatter-' + prefOrCity);
  let leftDataset, leftStatName, leftUnit;
  let rightDataset, rightStatName, rightUnit;
  if (isEStat) {
    if (!leftVal.statData.length) return;
    leftDataset = JSON.parse(JSON.stringify(leftVal.statData));
    leftStatName = leftVal.statName;
    leftUnit = leftVal.unit;
    if (!rightVal.statData.length) return;
    rightDataset = JSON.parse(JSON.stringify(rightVal.statData));
    rightStatName = rightVal.statName;
    rightUnit = rightVal.unit;
  } else {
    if (!leftVal.statData.data) return;
    leftDataset = JSON.parse(JSON.stringify(leftVal.statData.data));
    leftStatName = leftVal.statData.title;
    leftUnit = leftVal.statData.unit;
    if (!rightVal.statData.data) return;
    rightDataset = JSON.parse(JSON.stringify(rightVal.statData.data));
    rightStatName = rightVal.statData.title;
    rightUnit = rightVal.statData.unit;
  }
  ...
  // --------------------------------------------------------------------------------------------
  const year = isEStat ? mixDataset[mixDataset.length - 1].time.substr(0, 4) : 9999;
  ...
  class DataCreate {
    ...
    create () {
      ...
      const tgtLeftData = this.mixDataset.left;
      const tgtRightData = this.mixDataset.right;
      const leftDataAr = [];
      const rightDataAr = [];
      const kaikiData = [];
      // ４７都道府県でループ
  ...
  const zoomed = () => {
    ...
    let value;
    if (isEStat) {
      value = Number(d3.select('#year-range-' + prefOrCity).select('.year-range').property("value"));
      const year = mixDataset[value].time.substr(0, 4);
      rangeDiv.select('.year-range-text').text(year);
    } else {
      value = 0 // 宮崎県市町村は1年しかないから
    }
