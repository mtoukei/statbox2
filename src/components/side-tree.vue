  ...
  export default {
    ...
    methods: {
      ...
      nodeClickEstat1 (e) {
        if (!e.children) {
          this.$store.commit('statList/transitionSet', true);
          this.$store.commit('statList/selectStatEstat', {statId: e.statId, side: this.side, statName: e.label, unit: e.unit, prefOrCity: 'pref' })
        }
      },
      //-----------------------------------------------------------------------------------------
      // 都道府県時系列
...
<style lang="scss">
    ...
    .tree-div {
        box-sizing: border-box;
        height: calc(100vh - #{$header-menu-height} - #{$footer-height} - 120px);
        padding-top: 10px;
        padding-bottom: 50px;
        overflow: auto;
    }
    .pref-select, .city-select {
        width:80px;
    }
    #left-side-city .pref-select, #left-side-scatter-city .pref-select {
        width: 100px;
    }
    .side-sourse {
        margin-top: 10px;
